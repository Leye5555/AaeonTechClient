...
const Response = ({ pathname, onClick, action }) => {
  const paymentStatus = useSelector((state) => state.makePayment);
    ...
    <Div>
      <div className="lds-ellipsis">
        {!paymentStatus?.message ? (
          ...
          <div className="lds-ellipsis" >
            <div className="message"><p style={{fontSize : "1.2rem"}}> {paymentStatus?.message}</p> </div>
            <section style={{display : "flex", flexFlow : "row nowrap", width : "100%", justifyContent : "space-between"}}>
              ...
              <h6 onClick={() => dispatch({type : "DELETE_RESPONSE", payload : {}})} style={{fontSize : "0.95rem", cursor : "pointer", padding : "0.5rem", borderRadius : "5px", backgroundColor : "#212121"}}>
                 {action}
              </h6>
            </Link>
